-- breakable_joint.lua - Box2D official Breakable Joint sample
-- Demonstrates joints that break when the constraint force exceeds a threshold.
...
local body_ids = {}
local joint_ids = {}
local break_force = 1000.0
local gravity_y = -10.0
...
function M.create_scene(world)
    ...
    motor_def.maxVelocityForce = 1000.0
    motor_def.maxVelocityTorque = 20.0
    table.insert(joint_ids, b2d.create_motor_joint(world, motor_def))
...
function M.update(world, dt)
    -- Check joint forces and break if needed
    for i, joint_id in ipairs(joint_ids) do
        if joint_id and b2d.joint_is_valid(joint_id) then
            ...
            local force_mag_sq = force[1] * force[1] + force[2] * force[2]
            if force_mag_sq > break_force * break_force then
...
function M.update_gui(world)
    imgui.begin_window("Breakable Joint")

    local changed
    changed, break_force = imgui.slider_float("Break Force", break_force, 0, 10000)

    changed, gravity_y = imgui.slider_float("Gravity", gravity_y, -50, 50)
    if changed then
        b2d.world_set_gravity(world, {0, gravity_y})
    end
...
function M.render(camera, world)
    ...
    for i, joint_id in ipairs(joint_ids) do
        if joint_id and b2d.joint_is_valid(joint_id) then
            local force = b2d.joint_get_constraint_force(joint_id)
            -- Visual indicator at anchor point
